...
BEGIN_MESSAGE_MAP(CImageToolDoc, CDocument)
	...
	ON_UPDATE_COMMAND_UI(ID_OPENCV_CANNY, &CImageToolDoc::OnUpdateOpencvCanny)
	ON_COMMAND(ID_FACE_DETECTION, &CImageToolDoc::OnFaceDetection)
	ON_UPDATE_COMMAND_UI(ID_FACE_DETECTION, &CImageToolDoc::OnUpdateFaceDetection)
END_MESSAGE_MAP()
...
void CImageToolDoc::OnOpencvCanny()
{
	...
	AfxPrintInfo(_T("[OpenCV/Canny] 입력 영상: %s, Low Th: %4.2lf, High Th: %4.2lf"),
		GetTitle(), lowTh, highTh);
	AfxNewBitmap(dib);
}


void CImageToolDoc::OnUpdateOpencvCanny(CCmdUI *pCmdUI)
{
	pCmdUI->Enable(m_Dib.GetBitCount() == 8);
}


void CImageToolDoc::OnFaceDetection()
{
	...
	face_cascade.detectMultiScale(mat, faces, 1.1, 2, CV_HAAR_SCALE_IMAGE, Size(30, 30));

	for (Rect face : faces)
	{
		rectangle(mat, face, Scalar(0, 0, 0), 2);
	}
...
void CImageToolDoc::OnUpdateFaceDetection(CCmdUI *pCmdUI)
{
	pCmdUI->Enable(m_Dib.GetBitCount() == 8);
}
